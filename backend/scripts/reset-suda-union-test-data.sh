...
-- 说明：
-- - 该脚本用于“联调测试环境强制覆盖数据”，因此采用 drop + recreate；
-- - 避免历史残留 schema（缺列/列名不一致）导致 Web-only 认证与活动同步查询失败。
DROP TABLE IF EXISTS suda_activity_apply;
DROP TABLE IF EXISTS suda_activity;
DROP TABLE IF EXISTS suda_user;
