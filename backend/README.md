...
3. 本地联调脚本怎么跑
4. 如果你想“一条命令把前后端一起拉起来”，应该回仓库根目录看哪里
...
- 只想单机快速演示后端：看 `2) 仅后端 Docker Compose（可选，排障/拆分部署备用）`
...
## 2) 仅后端 Docker Compose（可选，排障/拆分部署备用）

> 如果你的目标是“一条命令把前后端都跑起来”，不要在 `backend/` 目录执行 compose。
> 请回到仓库根目录，使用根 `README.md` 里的“全栈 Docker Compose 一键启动”。

适合：你只想快速拉起 **MySQL + Redis + Backend** 做后端排障，前端由外部 Nginx / 已有静态资源托管负责。
