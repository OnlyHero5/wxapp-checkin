...
- FR-011 普通用户活动列表仅返回并展示“已报名或已参加”活动（`my_registered=true` 或 `my_checked_in=true`）
- FR-012 普通用户卡片显示“我的状态”（`已报名` / `已参加`）
- FR-013 工作人员卡片显示签到按钮（仅进行中）
- FR-014 `support_checkout=true` 时显示签退按钮（仅进行中）
- FR-015 `has_detail=true` 时显示详情按钮
- FR-016 已完成活动不允许签到/签退（前端与后端双重约束）
- FR-017 工作人员卡片显示 `checkin_count`

### 6.4 活动详情
- FR-018 详情页展示：名称、类型、开始时间、地点、已签到人数、描述
- FR-019 普通用户仅可查看“已报名或已参加”活动详情，禁止查看未报名未参加活动
- FR-020 无网时需提示“可能不是最新”

### 6.5 个人中心
- FR-021 展示基础资料：姓名、学号、学院/部门、社团/组织、绑定状态
- FR-022 普通用户展示社会分与讲座分
- FR-023 当前版本不展示“曾参加活动”模块

### 6.6 历史记录能力（当前版本状态）
- FR-024 独立“签到记录列表/详情页”入口在当前版本已下线
- FR-025 `/api/checkin/records*` 接口保留为兼容能力，非当前主验收链路
...
- 活动列表接口: `progress_status`, `support_checkout`, `has_detail`, `checkin_count`, `my_registered`, `my_checked_in`
...
- 2026-02-08：普通用户活动可见性收敛为“已报名或已参加”，详情接口增加同口径鉴权。
