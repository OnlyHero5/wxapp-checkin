...
### 6.2A 角色分流与活动卡片
- FR-011A 登录后按 `role` 分流显示能力
- FR-011B 普通用户活动页显示活动卡片与详情，不显示签到/签退按钮
- FR-011C 工作人员活动页卡片需支持按钮：签到、签退（可选）、详情（可选）
- FR-011D 普通用户活动卡片展示“我的签到状态”，不展示活动总签到人数
- FR-011E 工作人员活动卡片展示活动总签到人数
...
### 6.6 个人中心扩展
- FR-021 个人中心展示姓名、学号、学院/部门、社团/组织、绑定状态
- FR-022 普通用户个人中心展示社会分与讲座分
- FR-023 社会分与讲座分由后端返回，不由前端计算
- FR-024 普通用户个人中心不显示“曾参加活动”卡片
...
### 6.5 UI 与视觉
- FR-018 使用 TDesign（腾讯官方）或等价现代化组件库
...
- `role` 角色（`normal` / `staff`）  
- `permissions` 权限数组  
- `user_profile` 用户资料（含 `social_score`、`lecture_score`）  
...
### 9.7 活动列表（角色分流）
用途: 提供活动卡片列表  
接口: `GET /api/staff/activities`  
请求字段:
- `session_token`  
响应字段:
- `activities[]`  
- `activity_id` `activity_title` `activity_type` `start_time` `location`  
- `support_checkout` `has_detail` `description`  
- `checkin_count`（工作人员视角）  
- `my_checked_in`（普通用户视角）  

### 9.8 工作人员活动动作
用途: 工作人员执行签到/签退  
接口: `POST /api/staff/activity-action`  
请求字段:
- `session_token`  
- `activity_id`  
...
- `qr_token`  
响应字段:
- `status` `message` `checkin_record_id`  
...
## 14. 更新记录
- 2026-02-04：组件库调整为 TDesign（腾讯官方）；视觉主题更新为“石墨雾”，主按钮改为深蓝并保持居中；构建流程与发布信息见 README/changes。
- 2026-02-06：新增角色分流与活动卡片能力；普通用户显示社会分/讲座分与“我的签到状态”；更新后端接口字段需求。
