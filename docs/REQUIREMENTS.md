...
- 动态二维码展示与扫码提交
- 活动可见性与权限控制
- Web 管理系统主库（`suda_union`）与小程序扩展库协同同步（不破坏原有主库结构）
...
- 错误状态提示与重试
- Java 后端服务与扩展库表（`wx_*`）
- 同步任务（`suda_union` -> 扩展库投影、扩展库 outbox -> `suda_union` 回写）
- Linux 服务器部署与运行脚本
...
### 6.8 后端与数据同步
- FR-033 在不修改 `suda_union` 主表结构前提下，通过扩展表实现微信身份、会话、二维码和事件模型
...
- NFR-007 二维码票据需具备防篡改/防伪造能力：后端使用 `QR_SIGNING_KEY` 对 nonce 做 HMAC 签名，consume 侧验签失败必须拒绝
- NFR-008 二维码相关表需具备“数据保留 + 定期清理”机制，禁止 `wx_qr_issue_log`/`wx_replay_guard` 无限增长；生产可关闭 issue log 写入
...
## 10. 更新记录
- 2026-02-28：二维码生产加固（Point 2/3/4）：signed nonce（`QR_SIGNING_KEY` 生效）、issue log 可禁用 + retention cleanup、rotate/grace override 口径一致。
