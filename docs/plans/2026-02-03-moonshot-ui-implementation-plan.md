# Moonshot Miniapp UI Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Apply the Moonshot Obsidian Grid dark UI across the mini program, add a minimal “个人中心” tab page, and integrate Ant Design Mini Program components while logging all changes in `docs/changes.md`.

**Architecture:** Centralize visual tokens in `src/app.wxss`, refactor four existing pages’ WXML to use AntD components where possible, and add a new profile page with minimal identity display using existing storage. Maintain existing business logic in page JS, only adjusting UI bindings as needed.

**Tech Stack:** WeChat Mini Program (WXML/WXSS/JS), Ant Design Mini Program (`@ant-design/mini`), existing utils/storage/auth/api.

---

### Task 1: Sync project sources into the worktree

**Files:**
- Copy: `src/`
- Copy: `project.config.json`
- Copy: `project.private.config.json`
- Copy: `docs/changes.md`
- Copy: `design-system/`

**Step 1: Copy core project files from main working tree**

```powershell
$src = 'H:\wechat-checkin\wxapp-checkin'
$dst = 'H:\wechat-checkin\wxapp-checkin\.worktrees\moonshot-ui'
Copy-Item "$src\src" "$dst\src" -Recurse -Force
Copy-Item "$src\project.config.json" "$dst\project.config.json" -Force
Copy-Item "$src\project.private.config.json" "$dst\project.private.config.json" -Force
Copy-Item "$src\docs\changes.md" "$dst\docs\changes.md" -Force
Copy-Item "$src\design-system" "$dst\design-system" -Recurse -Force
```
...
**Step 3: Commit baseline project files**

```bash
git add src project.config.json project.private.config.json docs/changes.md design-system

git commit -m "chore: add miniapp baseline files"
```

---

### Task 2: Add Ant Design Mini Program dependency

**Files:**
- Create: `package.json`
- Modify: `.gitignore`

**Step 1: Add `package.json` with AntD dependency**
...
```json
{
  ...
  "version": "0.1.0",
  "dependencies": {
    "@ant-design/mini": "latest"
  }
}
```

**Step 2: Ignore generated folders**
...
**Step 3: Install npm deps and build in WeChat DevTools (manual)**

Run:
```
npm install
```
Then in WeChat DevTools: **Tools → Build NPM**.
...
git commit -m "chore: add Ant Design Mini Program dependency"
```

---

### Task 3: Apply global Obsidian Grid theme
...
### Task 4: Update app.json for new tab
...
<view class="page page-home">
  <view class="content">
    ...
    <ant-button class="btn-primary" type="primary" bindtap="onScan" disabled="{{!isOnline || loading}}">
      {{loading ? '正在验证...' : '扫码签到'}}
    </ant-button>
  ...
  <view class="success-overlay" wx:if="{{showSuccess}}" bindtap="closeSuccess">
    <view class="success-card">
      ...
      <ant-button class="btn-ghost" bindtap="goRecords">查看记录</ant-button>
    </view>
  </view>
</view>
```

**Step 2: Register AntD components**
...
```json
{
  ...
  "usingComponents": {
    ...
    "ant-card": "@ant-design/mini/es/card/index"
  }
}
```
...
git commit -m "feat: restyle index page with AntD components"
```

---

### Task 6: Refactor 记录页 UI
...
<view class="page page-records">
  <view class="content">
    ...
    <ant-card wx:for="{{records}}" wx:key="record_id" class="list-card" bindtap="openDetail" data-id="{{item.record_id}}">
      <view class="info-row">
        <text class="list-title">{{item.activity_title}}</text>
        <view class="tag">已签</view>
      </view>
...
git commit -m "feat: restyle records page with AntD cards"
```

---

### Task 7: Refactor 详情页 UI
...
<view class="page page-detail">
  <view class="content">
    ...
    <ant-button class="btn-ghost" bindtap="goBack">返回</ant-button>
  </view>
</view>
```
...
git commit -m "feat: restyle record detail page"
```

---

### Task 8: Refactor 注册页 UI
  ...
  <view class="content">
    ...
    <ant-card class="card">
      <view class="info-row">
        ...
        <view class="tag">{{wxIdentity ? '已识别' : '未获取'}}</view>
      </view>
      <view class="divider"></view>
      ...
      <ant-button class="btn-primary" type="primary" bindtap="onSubmit" disabled="{{!isOnline || submitting}}">
        {{submitting ? '提交中...' : '完成绑定'}}
      </ant-button>
    </ant-card>
  </view>
...
git commit -m "feat: restyle register page"
```

---

### Task 9: Add 个人中心 page
  ...
  <view class="content">
    ...
    <ant-card class="card">
      ...
      <view class="info-row">
        ...
        <view class="tag">{{isBound ? '已绑定' : '未绑定'}}</view>
      </view>
      <ant-button class="btn-primary" type="primary" bindtap="goBind">{{isBound ? '重新绑定' : '去绑定'}}</ant-button>
    </ant-card>
  </view>
</view>
```
...
### Task 10: Update `docs/changes.md`
...
- Ant Design Mini Program dependency added
- Updated four pages to dark minimal UI + AntD components
- Added 个人中心 tab/page
...
- Follow `docs/plans/2026-02-03-moonshot-miniapp-design.md` for visual intent.
